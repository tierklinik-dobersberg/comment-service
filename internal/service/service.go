...
func (svc *Service) CreateScope(ctx context.Context, req *connect.Request[commentv1.CreateScopeRequest]) (*connect.Response[commentv1.CreateScopeResponse], error) {
	...
	req.Msg.Id = ""
	if req.Msg.Id != "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, fmt.Errorf("ID must be autogenerated"))
	}

	insertId, err := svc.Repository.CreateScope(ctx, scopeModel)
	if err != nil {
		return nil, err
	}

	scope, err := svc.Repository.GetScopeByID(ctx, insertId)
	if err != nil {
		return nil, err
	}
...
func (svc *Service) CreateComment(ctx context.Context, req *connect.Request[commentv1.CreateCommentRequest]) (*connect.Response[commentv1.CreateCommentResponse], error) {
	...
	case *commentv1.CreateCommentRequest_Scope:
		m.Scope = v.Scope
...
func (svc *Service) ListComments(ctx context.Context, req *connect.Request[commentv1.ListCommentsRequest]) (*connect.Response[commentv1.ListCommentsResponse], error) {
	trees, err := svc.Repository.GetCommentTreeByScope(ctx, req.Msg.Scope)
	if err != nil {
		return nil, err
	}
	...
	for idx, tree := range trees {
		...
		minifyTree(res.Result[idx])
	}

	return connect.NewResponse(res), nil
}

func minifyTree(pb *commentv1.CommentTree) {
	pruneKeys := []string{
		"scope",
		"parent_id",
	}
